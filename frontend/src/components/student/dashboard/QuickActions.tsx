...
const quickActions: QuickActionItem[] = [
  {
    ...
    desc: 'Créer une démarche',
    icon: FilePlus2,
    to: '/student/requests?new=1',
    tile: 'bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400',
  },
  {
    id: 'qa-2',
    label: 'Mes documents',
    desc: 'Voir mes fichiers',
    icon: FolderOpen,
...
export default function QuickActions() {
  ...
  return (
    <div className="card p-6">
      <h3 className="mb-5 text-[15px] font-bold text-slate-800 dark:text-slate-100">
        Actions rapides
      </h3>
          ...
          return (
            ...
            >
              <button
                ...
                disabled={action.id === 'qa-3' && downloading}
                className="group relative flex w-full flex-col items-center justify-center gap-2.5 rounded-2xl border border-slate-200/70 bg-gradient-to-br from-white to-slate-50 px-4 py-5 text-center transition-all duration-200 hover:-translate-y-1 hover:border-primary-300 hover:shadow-soft-hover dark:border-slate-800 dark:from-slate-900 dark:to-slate-800/50 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-xl shadow-sm transition-all duration-200 group-hover:scale-110 ${action.tile}`}
                >
                  {action.id === 'qa-3' && downloading ? (
                    <Loader2 size={20} className="animate-spin" />
                  ) : (
                    <ItemIcon size={20} />
                  )}
                </div>
                <span className="text-[12.5px] font-semibold leading-tight text-slate-700 transition-colors group-hover:text-primary-700 dark:text-slate-300 dark:group-hover:text-primary-300">
                  {action.label}
                </span>
