...
const MODULE_CONFIG: Record<string, { icon: any }> = {
    Authentification: { icon: ShieldCheck },
    Profil: { icon: UserCog },
    Demandes: { icon: FileText },
    Réclamations: { icon: MessageSquareWarning },
    Documents: { icon: FolderArchive },
    Messagerie: { icon: Mail },
    Calendrier: { icon: CalendarRange },
    "Assistant IA": { icon: Sparkles },
    Notifications: { icon: BellRing },
    Administration: { icon: Shield },
    Scolarité: { icon: GraduationCap },
    Général: { icon: Settings2 },
};

const getModuleIcon = (module: string) =>
    (MODULE_CONFIG[module] || MODULE_CONFIG["Général"]).icon;
...
const getRoleStyle = (roleName: string) =>
    ROLE_STYLES[roleName] || { label: "Personnalisé", icon: Shield };
...
export default function AdminPermissions() {
    ...
    const selectedRoleStyle = useMemo(
        () => (selectedRole ? getRoleStyle(selectedRole.nom_role) : null),
        [selectedRole]
    );
    ...
    const selectedPerms = useMemo(() => {
        ...
        return (Array.isArray(permissions) ? permissions : []).filter((p) =>
            ids.includes(p.id_permission)
        );
    ...
    const totalModules = useMemo(
        () => [...new Set(permissions.map((p) => p.module || "Général"))].length,
        [permissions]
    );
    const totalAssignments = useMemo(
        () => Object.values(rolePermissionsMap).reduce((sum, arr) => sum + (arr?.length || 0), 0),
        [rolePermissionsMap]
    );
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-slate-100 dark:bg-slate-800">
                                            <RoleIcon size={14} className="text-slate-600 dark:text-slate-400" />
                                        </div>
                                        ...
                                        <span className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] font-bold tabular-nums text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                                            {count}
                                        </span>
                                        <ChevronRight size={13} className={`text-slate-400 transition ${isSelected ? "opacity-100" : "opacity-0"}`} />
                                    </button>
                                );
                            })
                        )}
                ...
                <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                    {selectedRole ? (
                        <>
                                ...
                                <div className="flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-3">
                                        <div className="flex h-10 w-10 items-center justify-center rounded-md bg-slate-100 dark:bg-slate-800">
                                            <SelectedRoleIcon size={17} className="text-slate-600 dark:text-slate-400" />
                                        </div>
                                        <div>
                                            ...
                                            <p className="text-[11px] text-slate-500 dark:text-slate-400">
                                                {selectedRoleStyle?.label} · {selectedPerms.length} permission{selectedPerms.length !== 1 ? "s" : ""}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="relative w-64">
                                        ...
                                        <input
                                            ...
                                            className="w-full rounded-md border border-slate-200 bg-white py-1.5 pl-8 pr-8 text-[12px] text-slate-800 placeholder:text-slate-400 focus:border-slate-400 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                                        />
                                        {search && (
                                            <button
                                                onClick={() => setSearch("")}
                            ...
                            <div className={`flex-1 overflow-y-auto p-5 ${SCROLLBAR_CLASSES}`}>
                                {filteredSelectedPerms.length === 0 ? (
                                    <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
                                        ...
                                        <p className="text-[13px] font-medium text-slate-900 dark:text-white">
                                            {search ? "Aucun résultat" : "Aucune permission"}
                                        </p>
                                        <p className="text-[11px] text-slate-500 dark:text-slate-400">
                                            {search
                                                ...
                                                <div key={module}>
                                                    <div className="mb-2 flex items-center gap-2">
                                                        <div className="flex h-5 w-5 items-center justify-center rounded bg-slate-100 dark:bg-slate-800">
                                                            <Icon size={11} className="text-slate-600 dark:text-slate-400" />
                                                        </div>
                                                    ...
                                                    <div className="overflow-hidden rounded-md border border-slate-200 dark:border-slate-800">
                                                        {safePerms.map((perm: any, idx: number) => (
                                                            <div
                                                                key={perm.id_permission || idx}
                                                                className={`flex items-start gap-3 bg-white px-4 py-2.5 dark:bg-slate-900 ${
                                                                    idx !== safePerms.length - 1
                                                                        ? "border-b border-slate-100 dark:border-slate-800"
                                                                        : ""
                                                                }`}
                                                            >
                                                                <div className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-sm bg-emerald-500 text-white">
                                                                    <Check size={9} strokeWidth={3} />
                                                                </div>
                                                                <div className="min-w-0 flex-1">
                                                                    <p className="font-mono text-[11.5px] font-semibold text-slate-800 dark:text-slate-200">
                                                                        {perm.nom_permission}
                                                                    </p>
                                                                    {perm.description && (
                        ...
                        <div className="flex h-full items-center justify-center">
                            <p className="text-[12px] text-slate-400">Sélectionnez un rôle</p>
                        </div>
                    )}
                </section>
            </div>

            <footer className="flex shrink-0 items-center justify-between border-t border-slate-200 pt-3 text-[11px] text-slate-400 dark:border-slate-800 dark:text-slate-500">
                <div className="flex items-center gap-1.5">
                    <span className="inline-flex h-3 w-3 items-center justify-center rounded-sm bg-emerald-500 text-white">
