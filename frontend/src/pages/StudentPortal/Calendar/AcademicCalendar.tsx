...
import {
  BookOpen, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight,
  FileText, GraduationCap, MoonStar, PenLine, Presentation, Umbrella, Loader2,
} from "lucide-react";
...
function isPast(e: CalendrierEvent, today: Date) {
  return eventEnd(e).getTime() < today.getTime();
}
...
function formatRange(e: CalendrierEvent) {
  if (e.start === e.end || !e.end) return formatShort(e.start);
  return `${formatShort(e.start)} → ${formatShort(e.end)}`;
}
...
export default function AcademicCalendar() {
  ...
  const [filiere, setFiliere] = useState<{ id: string; nom: string } | null>(null);
  ...
  const filteredEvents = useMemo(() => {
    return events
      .filter((e) => !year || e.annee === year || !e.annee)
      .filter((e) => semester === "all" || e.semester === semester || e.semester === "all")
      .sort((a, b) => parseDate(a.start).getTime() - parseDate(b.start).getTime());
  }, [events, year, semester]);

  const next = filteredEvents.find((e) => eventEnd(e).getTime() >= today.getTime());

  const s1Count = filteredEvents.filter((e) => e.semester === "S1").length;
  const s2Count = filteredEvents.filter((e) => e.semester === "S2").length;
  const otherCount = filteredEvents.length - s1Count - s2Count;
  ...
  const upcoming = filteredEvents.filter((e) => eventEnd(e).getTime() >= today.getTime()).slice(0, 5);

  const jumpTo = (e: CalendrierEvent) => {
    const s = parseDate(e.start);
    setCursor(new Date(s.getFullYear(), s.getMonth(), 1));
    setSelectedDay(s);
  };

  const statusOf = (e: CalendrierEvent) => {
    if (isOngoing(e, today))
      return { label: "En cours", cls: "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" };
    if (isPast(e, today))
      return { label: "Terminé", cls: "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400" };
    return { label: `J-${daysUntil(e, today)}`, cls: "bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300" };
  };

  const getCurrentPhase = () => {
    const ongoing = filteredEvents.find((e) => isOngoing(e, today));
    if (ongoing) return ongoing.title;
    return "Période d'enseignement";
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Chargement du calendrier...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <button onClick={loadCalendrier} className="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm">
            Réessayer
          </button>
        </div>
      </div>
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[12px] text-slate-500 dark:text-slate-400">
            {filiere ? `Filière : ${filiere.nom}` : "Dates clés de l'année académique"}
          </p>
        </div>
            ...
            {annees.map((y) => (
              <button
                ...
                className={`relative rounded-md px-3 py-1 text-[11px] font-semibold transition ${
                  year === y ? "text-primary-700 dark:text-primary-300" : "text-slate-500 hover:text-slate-700 dark:text-slate-400"
                }`}
              >
                {year === y && (
                  <motion.span
        ...
        <div className="grid grid-cols-1 gap-px bg-slate-200 sm:grid-cols-3 dark:bg-slate-800">
          <div className="flex items-center justify-between gap-3 bg-white px-5 py-4 dark:bg-slate-900">
            <div className="min-w-0">
              <p className="text-[9.5px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Phase actuelle</p>
              <p className="mt-1 flex items-center gap-1.5 text-[13px] font-semibold text-slate-900 dark:text-white">
                <span className="h-1.5 w-1.5 shrink-0 animate-pulse rounded-full bg-emerald-500" />
                <span className="truncate">{getCurrentPhase()}</span>
              </p>
              <p className="mt-0.5 text-[10.5px] text-slate-500 dark:text-slate-400">Année {year}</p>
            </div>
          ...
          <div className="flex items-center justify-between gap-3 bg-white px-5 py-4 dark:bg-slate-900">
            <div className="min-w-0">
              <p className="text-[9.5px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Prochain événement</p>
              {next ? (
                <>
                  <p className="mt-1 truncate text-[13px] font-semibold text-slate-900 dark:text-white">{next.title}</p>
                ...
                <p className="mt-1 text-[13px] font-semibold text-slate-400 dark:text-slate-500">Aucun à venir</p>
              )}
            </div>
          ...
          <div className="flex items-center justify-between gap-3 bg-white px-5 py-4 dark:bg-slate-900">
            <div className="min-w-0">
              <p className="text-[9.5px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Événements</p>
            ...
            <div className="flex h-9 items-end gap-0.5">
              <span className="w-1.5 rounded-sm bg-blue-400" style={{ height: `${Math.max(20, (s1Count / maxSem) * 100)}%` }} />
              <span className="w-1.5 rounded-sm bg-emerald-400" style={{ height: `${Math.max(20, (s2Count / maxSem) * 100)}%` }} />
              <span className="w-1.5 rounded-sm bg-slate-300 dark:bg-slate-600" style={{ height: `${Math.max(20, (otherCount / maxSem) * 100)}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        <div className="card p-4 xl:col-span-2">
          <div className="mb-3 flex items-center justify-between gap-2.5">
              ...
              {(["all", "S1", "S2"] as const).map((s) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1 text-[10px] font-semibold transition ${
                    semester === s ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "text-slate-500 hover:text-slate-700"
                  }`}
                >
                  {s === "all" ? "Tous" : s}
                </button>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800"
              >
                <ChevronLeft size={13} />
              </button>
              <button
                onClick={() => { setCursor(new Date(today.getFullYear(), today.getMonth(), 1)); setSelectedDay(today); }}
                className="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[10px] font-semibold text-primary-600 transition hover:bg-primary-50 dark:border-slate-700 dark:bg-slate-800"
              >
                Aujourd'hui
              </button>
              <button
          ...
          <div className="grid grid-cols-7 text-center text-[9px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
            {WEEKDAYS.map((d) => <span key={d} className="py-1.5">{d}</span>)}
          </div>
              ...
              return (
                <button
                  ...
                  className={`flex h-12 flex-col items-center justify-center gap-0.5 rounded-lg text-[11.5px] font-medium transition sm:h-14 ${
                    ...
                    isToday ? "bg-primary-50 font-bold text-primary-700 ring-1 ring-primary-200 dark:bg-primary-900/20 dark:text-primary-300" :
                    `${weekend && inMonth ? "bg-slate-50/70 dark:bg-slate-800/30" : ""} text-slate-600 hover:bg-slate-100 dark:text-slate-300`
                  } ${!inMonth ? "opacity-30" : ""}`}
                >
                  <span className="tabular-nums">{day.getDate()}</span>
                  <span className="flex gap-0.5">
                    {dots.slice(0, 3).map((e) => (
                      <span key={e.id} className={`h-1 w-1 rounded-full ${isSelected ? "bg-white" : EVENT_TYPE_CONFIG[e.type].dot}`} />
                    ))}
                  </span>
                </button>
              );
          ...
          <div className="mt-3 flex flex-wrap gap-1.5 border-t border-slate-100 pt-3 dark:border-slate-800">
            {(Object.keys(EVENT_TYPE_CONFIG) as EventType[]).map((t) => (
              <span key={t} className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[9.5px] font-medium text-slate-500 dark:border-slate-700 dark:bg-slate-800">
        ...
        <div className="space-y-5">
          <div className="card overflow-hidden p-0">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-[400px] overflow-y-auto">
                ...
                return (
                  <div key={e.id} className="flex items-start gap-2.5 px-4 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[12px] font-semibold text-slate-800 dark:text-slate-100">{e.title}</p>
                      <p className="mt-0.5 text-[10.5px] tabular-nums text-slate-500 dark:text-slate-400">
                        {formatRange(e)}
                        {e.note ? ` · ${e.note}` : ""}
                      </p>
                      <div className="mt-1 flex items-center gap-1">
                        <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold tabular-nums ${st.cls}`}>{st.label}</span>
                        <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-500 dark:bg-slate-800">{cfg.label}</span>
                      </div>
                    </div>
                  </div>
                );
              ...
              {selectedEvents.length === 0 && (
                <p className="px-4 py-6 text-center text-[12px] text-slate-400 dark:text-slate-500">Aucun événement ce jour.</p>
              )}
            </div>
          </div>
          ...
          <div className="card p-4">
            <div className="mb-2.5 flex items-center justify-between">
              <h3 className="text-[12.5px] font-bold text-slate-800 dark:text-white">Prochains événements</h3>
              <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[9.5px] font-bold tabular-nums text-slate-500 dark:bg-slate-800">{upcoming.length}</span>
            </div>
            <div className="space-y-0.5">
              {upcoming.map((e) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[11.5px] font-semibold text-slate-700 dark:text-slate-200">{e.title}</p>
                      <p className="text-[9.5px] tabular-nums text-slate-400 dark:text-slate-500">{formatShort(e.start)}</p>
                    </div>
              ...
              {upcoming.length === 0 && <p className="py-3 text-center text-[12px] text-slate-400 dark:text-slate-500">Aucun événement à venir.</p>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-1 text-[10px] font-medium text-slate-400 dark:text-slate-500">
        <span className="tabular-nums">{filteredEvents.length} sur {events.length} événements affichés</span>
