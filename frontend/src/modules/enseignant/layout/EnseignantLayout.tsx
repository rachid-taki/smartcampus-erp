...
import {
  ...
  ChevronsLeft, ChevronsRight, Moon, Sun, MessageSquare, UserX, 
  CalendarDays, Building2, GraduationCap, BookOpen, WifiOff, Clock, LayoutGrid
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// Imports propres au module Enseignant
...
import ReservationEnseignant from '../pages/ReservationEnseignant';

const API_BASE = 'http://localhost:3000/api';
const NOTIF_POLL_INTERVAL_MS = 30000; // 30s

type TabKey = 'seances' | 'reclamations' | 'absences' | 'consultations' | 'reservations';
...
// ─── NAV GROUPS : Organisation en accordéons ───
const NAV_GROUPS = [
  {
    title: 'Pédagogie & Cours',
    items: [
      { key: 'seances', label: 'Mes Séances', icon: BookOpen },
      { key: 'consultations', label: 'Planification Consultations', icon: CalendarDays },
    ]
  },
  {
    title: 'Évaluations & Suivi',
    items: [
  ...
  },
  {
    ...
    items: [
      { key: 'reservations', label: 'Réservation de Salles', icon: Building2 },
    ]
  }
];

const PAGE_TITLES: Record<TabKey, { title: string; subtitle: string }> = {
  seances: { title: 'Mes Séances', subtitle: 'Vos cours et emplois du temps' },
  reclamations: { title: 'Réclamations de Notes', subtitle: 'Traitement des demandes de révision' },
  absences: { title: 'Validation des Absences', subtitle: 'Justificatifs et suivi des étudiants' },
  consultations: { title: 'Planification des Consultations', subtitle: 'Créneaux de disponibilité enseignant' },
  reservations: { title: 'Mes Réservations de Salles', subtitle: "Demandes d'espaces pour vos cours" },
};
...
export default function EnseignantLayout() {
  ...
  const [activeTab, setActiveTab] = useState<TabKey>('seances');
  const [mobileOpen, setMobileOpen] = useState(false);
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('ens_sidebar') === '1');
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        setUser(parsedUser);
        setInitials(`${parsedUser.prenom?.[0] || ''}${parsedUser.nom?.[0] || ''}`.toUpperCase() || 'PR');
      } catch (e) {
  ...
  useEffect(() => { localStorage.setItem('ens_sidebar', collapsed ? '1' : '0'); }, [collapsed]);

  // 2. Gestion des clics en dehors des menus + touche Échap
  useEffect(() => {
  ...
  const renderPage = () => {
    switch (activeTab) {
      case 'seances': return <MesSeances />;
      case 'reclamations': return <ReclamationsNotes />;
      case 'absences': return <ValidationAbsences />;
      case 'consultations': return <PlanificationConsultations />;
      case 'reservations': return <ReservationEnseignant />;
      default: return null;
    }
  };
    ...
    <div className="h-screen w-full flex overflow-hidden bg-slate-50 dark:bg-slate-900">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 transition-all duration-300
        ...
        <div className={`flex items-center h-16 px-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 ${collapsed ? 'lg:justify-center lg:px-0' : 'gap-3'}`}>
          <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-sky-500 to-sky-700 shadow-md">
            <GraduationCap className="h-5 w-5 text-white" />
          </div>
          <div className={`min-w-0 ${collapsed ? 'lg:hidden' : ''}`}>
            <p className="text-sm font-bold text-slate-900 dark:text-white whitespace-nowrap">SmartCampus</p>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 whitespace-nowrap">Portail Enseignant</p>
          </div>
                        ...
                        return (
                          <button
                            ...
                            className={`group relative w-full flex items-center gap-3 rounded-lg py-2.5 text-sm font-medium transition-all
                              ...
                              ${active ? 'bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-400'
                                       : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/70'}`}
                          >
                            <span className={`absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-r-full bg-sky-500 transition-opacity ${active ? 'opacity-100' : 'opacity-0'}`} />
                            <Icon className={`h-[18px] w-[18px] flex-shrink-0 transition-colors ${active ? 'text-sky-600 dark:text-sky-400' : 'text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300'}`} />
          ...
          <div className={`px-3 pt-4 border-t border-slate-200 dark:border-slate-800 ${collapsed ? 'lg:px-0' : ''}`}>
            ...
            <button
              ...
              className={`group relative w-full flex items-start gap-3 rounded-lg py-2.5 text-sm font-medium transition-all duration-200 border
                ${collapsed
                  ? 'lg:justify-center lg:px-0 border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-300 hover:bg-sky-100 dark:hover:bg-sky-900/40'
                  : 'px-3 border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-300 hover:bg-sky-100 dark:hover:bg-sky-900/40 hover:border-sky-300 dark:hover:border-sky-700 shadow-sm'
                }`}
            >
              <LayoutGrid className="h-[18px] w-[18px] flex-shrink-0 mt-0.5" />
              <span className={`flex-1 text-left leading-snug ${collapsed ? 'lg:hidden' : ''}`}>Accueil & Portails</span>
            </button>
          </div>
        </nav>
        ...
        <div className={`border-t border-slate-200 dark:border-slate-800 p-3 ${collapsed ? 'lg:p-2' : ''}`}>
          <div className={`flex items-center gap-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 p-2.5 ${collapsed ? 'lg:justify-center' : ''}`}>
            <div className="relative flex-shrink-0">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-sky-700 text-xs font-bold text-white">{initials}</div>
            ...
            <div className={`min-w-0 ${collapsed ? 'lg:hidden' : ''}`}>
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{user?.role || 'Enseignant'}</p>
            </div>
          </div>
        </div>
      </aside>
        ...
        <header className="flex items-center justify-between h-16 px-4 sm:px-6 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 z-30">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 flex-1 justify-end">
            ...
            <div className="relative w-full max-w-sm hidden md:block" ref={searchRef}>
              <div className={`flex items-center gap-2 bg-slate-50 dark:bg-slate-800/50 border ${isSearchFocused ? 'border-sky-400 ring-2 ring-sky-500/20' : 'border-slate-200 dark:border-slate-700'} rounded-lg px-3 py-2 transition-all`}>
                    ...
                    <div className="p-2">
                      ...
                      {matchedModules.length > 0 ? (
                        matchedModules.map(mod => (
                          <button key={mod.key} onClick={() => go(mod.key as TabKey)} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-sky-50 dark:hover:bg-sky-900/20 hover:text-sky-700 dark:hover:text-sky-400 rounded-lg transition-colors">
                ...
                {notifOpen && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.15 }}
                    ...
                    <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                      ...
                      {unreadNotifsCount > 0 && (
                        <button onClick={markAllAsRead} className="text-[10px] font-semibold text-sky-600 dark:text-sky-400 hover:underline">
                    ...
                    <div className="max-h-72 overflow-y-auto custom-scrollbar">
                      {notifLoading ? (
                        ...
                        <div className="p-6 text-center text-slate-500">
                          ...
                          <button onClick={fetchNotifs} className="mt-2 text-[11px] font-semibold text-sky-600 dark:text-sky-400 hover:underline">
                            Réessayer
                          </button>
                        </div>
                        ...
                        <div
                          ...
                          className={`p-4 border-b last:border-0 border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer ${!n.lu ? 'bg-sky-50/30 dark:bg-sky-900/10' : ''}`}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">{n.titre}</p>
                            {!n.lu && <span className="h-1.5 w-1.5 rounded-full bg-sky-500 flex-shrink-0" />}
                          </div>
            ...
            <div className="relative" ref={profileRef}>
              <button onClick={() => setProfileOpen(!profileOpen)} aria-label="Menu du profil" className="flex items-center gap-2 pl-2 pr-1 py-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-sky-700 text-xs font-bold text-white shadow-sm">{initials}</div>
                ...
                {profileOpen && (
                  <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                    ...
                    <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-800">
                      <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{user?.prenom} {user?.nom}</p>
                      <p className="text-xs text-slate-500 truncate">{user?.email}</p>
                    </div>
                    <div className="p-1 space-y-0.5">
                      <button onClick={() => navigate('/profile')} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors">
                        <User className="h-4 w-4 text-slate-500" />Mon profil
                      </button>
                      <button onClick={() => navigate('/settings')} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors">
                        <Settings className="h-4 w-4 text-slate-500" />Paramètres
                      </button>
                    </div>
                    <div className="border-t border-slate-200 dark:border-slate-800 mt-1 pt-1 px-1">
