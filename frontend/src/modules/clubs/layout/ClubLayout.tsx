...
import {
  LayoutDashboard, Menu, X, Bell, ChevronDown,
  Search, Settings, LogOut, User, ChevronsLeft, ChevronsRight, Moon, Sun,
  Building2, Users, Award, Calendar, GraduationCap, WifiOff, Clock
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// Imports propres au module Clubs
...
const NOTIF_POLL_INTERVAL_MS = 30000; // 30s. Swap for a socket.io listener for true real-time.

type TabKey = 'dashboard' | 'clubs' | 'presidents' | 'evenements' | 'reservations';
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      { key: 'clubs', label: 'Gestion des Clubs', icon: Users },
      { key: 'presidents', label: 'Présidents', icon: Award },
      { key: 'evenements', label: 'Événements', icon: Calendar },
      { key: 'reservations', label: 'Réservation Salles', icon: Building2 },
    ]
  }
];

const PAGE_TITLES: Record<TabKey, { title: string; subtitle: string }> = {
  dashboard: { title: 'Tableau de bord', subtitle: 'Indicateurs de la vie étudiante' },
  clubs: { title: 'Gestion des Clubs', subtitle: 'Administration des entités para-universitaires' },
  presidents: { title: 'Présidents', subtitle: 'Suivi des mandats et responsables' },
  evenements: { title: 'Événements', subtitle: 'Planning et validation des activités' },
  reservations: { title: 'Réservation Salles', subtitle: "Demandes d'espaces pour les clubs" },
};
...
export default function ClubLayout() {
  ...
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('club_sidebar') === '1');
  ...
  const [initials, setInitials] = useState('BE');
  ...
  useEffect(() => { localStorage.setItem('club_sidebar', collapsed ? '1' : '0'); }, [collapsed]);
  ...
  switch (activeTab) {
    case 'dashboard':
      return <ClubsDashboard onNavigate={go} />;

    case 'clubs':
      return <GestionClubs />;

    case 'presidents':
      return <GestionPresidents />;
    ...
    <div className="h-screen w-full flex overflow-hidden bg-slate-50 dark:bg-slate-900">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 transition-all duration-300
        ...
        <div className={`flex items-center h-16 px-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 ${collapsed ? 'lg:justify-center lg:px-0' : 'gap-3'}`}>
          <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500 to-fuchsia-700 shadow-md">
            <Users className="h-5 w-5 text-white" />
          </div>
          <div className={`min-w-0 ${collapsed ? 'lg:hidden' : ''}`}>
            <p className="text-sm font-bold text-slate-900 dark:text-white whitespace-nowrap">SmartCampus</p>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 whitespace-nowrap">Vie Étudiante & Clubs</p>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-4 custom-scrollbar">
                        ...
                        return (
                          <button
                            ...
                            className={`group relative w-full flex items-center gap-3 rounded-lg py-2.5 text-sm font-medium transition-all
                              ...
                              ${active ? 'bg-fuchsia-50 dark:bg-fuchsia-900/20 text-fuchsia-700 dark:text-fuchsia-400'
                                       : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/70'}`}
                          >
                            <span className={`absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-r-full bg-fuchsia-500 transition-opacity ${active ? 'opacity-100' : 'opacity-0'}`} />
                            <Icon className={`h-[18px] w-[18px] flex-shrink-0 transition-colors ${active ? 'text-fuchsia-600 dark:text-fuchsia-400' : 'text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300'}`} />
          ...
          {/* Bouton de Navigation : Retour Scolarité */}
          <div className={`px-3 pt-4 border-t border-slate-200 dark:border-slate-800 ${collapsed ? 'lg:px-0' : ''}`}>
            ...
            <button
              ...
              title="Retour au portail Scolarité"
              className={`group relative w-full flex items-start gap-3 rounded-lg py-2.5 text-sm font-medium transition-all duration-200 border
                ${collapsed
                  ? 'lg:justify-center lg:px-0 border-fuchsia-200 dark:border-fuchsia-800 bg-fuchsia-50 dark:bg-fuchsia-900/20 text-fuchsia-700 dark:text-fuchsia-300 hover:bg-fuchsia-100 dark:hover:bg-fuchsia-900/40'
                  : 'px-3 border-fuchsia-200 dark:border-fuchsia-800 bg-fuchsia-50 dark:bg-fuchsia-900/20 text-fuchsia-700 dark:text-fuchsia-300 hover:bg-fuchsia-100 dark:hover:bg-fuchsia-900/40 hover:border-fuchsia-300 dark:hover:border-fuchsia-700 shadow-sm'
                }`}
            >
              <GraduationCap className="h-[18px] w-[18px] flex-shrink-0 mt-0.5" />
              <span className={`flex-1 text-left leading-snug ${collapsed ? 'lg:hidden' : ''}`}>Portail Scolarité</span>
            </button>
          </div>
        ...
        <div className={`border-t border-slate-200 dark:border-slate-800 p-3 ${collapsed ? 'lg:p-2' : ''}`}>
          <div className={`flex items-center gap-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 p-2.5 ${collapsed ? 'lg:justify-center' : ''}`}>
            <div className="relative flex-shrink-0">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-fuchsia-500 to-fuchsia-700 text-xs font-bold text-white">{initials}</div>
            ...
            <div className={`min-w-0 ${collapsed ? 'lg:hidden' : ''}`}>
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{user?.role || 'Bureau Étudiant'}</p>
            </div>
          </div>
        </div>
      </aside>
        ...
        <header className="flex items-center justify-between h-16 px-4 sm:px-6 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 z-30">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 flex-1 justify-end">
            ...
            <div className="relative w-full max-w-sm hidden md:block" ref={searchRef}>
              <div className={`flex items-center gap-2 bg-slate-50 dark:bg-slate-800/50 border ${isSearchFocused ? 'border-fuchsia-400 ring-2 ring-fuchsia-500/20' : 'border-slate-200 dark:border-slate-700'} rounded-lg px-3 py-2 transition-all`}>
                    ...
                    <div className="p-2">
                      ...
                      {matchedModules.length > 0 ? (
                        matchedModules.map(mod => (
                          <button key={mod.key} onClick={() => go(mod.key as TabKey)} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-900/20 hover:text-fuchsia-700 dark:hover:text-fuchsia-400 rounded-lg transition-colors">
                ...
                {notifOpen && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.15 }}
                    ...
                    <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                      ...
                      {unreadNotifsCount > 0 && (
                        <button onClick={markAllAsRead} className="text-[10px] font-semibold text-fuchsia-600 dark:text-fuchsia-400 hover:underline">
                    ...
                    <div className="max-h-72 overflow-y-auto custom-scrollbar">
                      {notifLoading ? (
                        ...
                        <div className="p-6 text-center text-slate-500">
                          ...
                          <button onClick={fetchNotifs} className="mt-2 text-[11px] font-semibold text-fuchsia-600 dark:text-fuchsia-400 hover:underline">
                            Réessayer
                          </button>
                        </div>
                        ...
                        <div
                          ...
                          className={`p-4 border-b last:border-0 border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer ${!n.lu ? 'bg-fuchsia-50/30 dark:bg-fuchsia-900/10' : ''}`}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">{n.titre}</p>
                            {!n.lu && <span className="h-1.5 w-1.5 rounded-full bg-fuchsia-500 flex-shrink-0" />}
                          </div>
            ...
            <div className="relative" ref={profileRef}>
              <button onClick={() => setProfileOpen(!profileOpen)} aria-label="Menu du profil" className="flex items-center gap-2 pl-2 pr-2 py-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-fuchsia-500 to-fuchsia-700 text-xs font-bold text-white">{initials}</div>
